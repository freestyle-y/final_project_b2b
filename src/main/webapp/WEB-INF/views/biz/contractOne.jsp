...
<body>
  ...
  <div class="contract-container">
    ...
    <div class="document-info">
      <div class="info-left">
        <div class="info-item">계약서 번호: <c:if test="${not empty contractOne}">${contractOne[0].contractNo}</c:if></div>
        <div class="info-item">계약일자: <span id="today"></span></div>
      </div>
      <div class="info-right">
        <div class="info-item">발행일: <span id="today2"></span></div>
        <div class="info-item">문서번호: <c:if test="${not empty contractOne}">${contractOne[0].contractNo}</c:if></div>
      </div>
    </div>
    ...
    <div class="contract-parties-section">
      ...
      <table class="contract-table">
        ...
        <tbody>
          ...
          <c:forEach var="con" items="${contractUser}">
          <tr>
            <td>을 (수요자)</td>
            <td>${con.companyName}</td>
            <td>${con.name}</td>
            <td>${con.phone }</td>
            <td>${con.address} ${con.detailAddress}</td>
          </tr>
          </c:forEach>
        </tbody>
      </table>
