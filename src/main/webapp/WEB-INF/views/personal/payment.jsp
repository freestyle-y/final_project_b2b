...
<head>
  ...
  <!-- Pretendard (KR) : 한글 기본 폰트 적용 -->
  <link rel="preconnect" href="https://cdn.jsdelivr.net" crossorigin>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/variable/pretendardvariable.css" crossorigin>
  ...
  <style>
    ...
    .num,.price,[data-price],#totalAmount,#finalAmount,#summaryTotal,#summaryFinal{
    ...
    /* 카드번호 4칸 UI(사용 안 해도 문제 없음) */
    .cardnum-group{display:flex;align-items:center;gap:8px;flex-wrap:nowrap}
    .cardnum-group .form-control{width:72px;text-align:center;letter-spacing:.08em}
    .cardnum-group .sep{opacity:.5;user-select:none}
    @media (max-width:400px){ .cardnum-group .form-control{width:64px} }
  </style>
</head>
...
<main class="main">
  <div class="page-title light-background">
    <div class="container d-lg-flex justify-content-between align-items-center">
      <h1 class="mb-2 mb-lg-0">결제</h1>
      <nav class="breadcrumbs">
        <ol>
          <li><a href="${pageContext.request.contextPath}/">홈</a></li>
          <li class="current">결제</li>
        </ol>
      </nav>
    </div>
  </div>
            ...
            <div class="card-body">
              ...
              <form id="paymentForm" class="checkout-form">
                        ...
                        <c:when test="${not empty cardList}">
                          ...
                          <div class="vstack gap-2 mb-3">
                            <c:forEach var="card" items="${cardList}">
                              <div class="form-check">
                                <input class="form-check-input" type="radio" name="cardNo" id="card-${card.paymentMethodNo}" value="${card.paymentMethodNo}" <c:if test="${card.isDefault == 'Y'}">checked</c:if>>
                        ...
                        <c:otherwise>
                          ...
                          <div id="newCardForm" class="border rounded p-3 bg-white">
                            <div class="row g-2">
                              ...
                              <div class="col-md-8"><label class="form-label">카드번호</label><input type="text" name="accountNumber" class="form-control"></div>
                              <div class="col-md-4"><label class="form-label">유효기간(YYYY-MM)</label><input type="month" name="cardExpiration" class="form-control"></div>
                              <div class="col-md-4"><label class="form-label">CVC</label><input type="text" name="cardCvc" maxlength="3" class="form-control"></div>
                              <div class="col-md-4"><label class="form-label">비밀번호 앞 2자리</label><input type="password" name="cardPassword" maxlength="2" class="form-control"></div>
                              <div class="col-12">
                                <div class="form-check">
                              ...
                              <div class="col-12">
                                <button type="button" class="btn btn-dark btn-sm" onclick="addCard()">
                ...
                <div class="checkout-section">
                  ...
                  <div class="section-content border rounded p-3 bg-light">
                    <c:forEach var="add" items="${mainAddress}">
                      ...
                      <p class="mb-0">배송지 명 : <strong id="selectedAddressNickname">${add.nickname}</strong></p>
                    </c:forEach>
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-body p-3">
                            <div class="table-responsive" style="max-height:60vh; overflow:auto;">
                              <table class="table table-sm align-middle text-center">
                                <thead class="table-light">
                                  <tr>
                                    <th>선택</th><th>주소</th><th>상세주소</th><th>우편번호</th><th>별칭</th><th>기본</th><th>삭제</th>
                                  </tr>
                                </thead>
                                    ...
                                    <tr>
                                      ...
                                      <td>
                                        <c:choose>
                                          ...
                                          <c:otherwise>
                                            <button type="button" class="btn btn-outline-primary btn-sm btn-set-main"
                                                    data-id="${address.addressNo}">
                                              기본지로
                                            </button>
                                          </c:otherwise>
                                        </c:choose>
                                      ...
                                      <td>
                                        <button type="button"
                                                class="btn btn-outline-danger btn-sm btn-del-address"
                                                data-id="${address.addressNo}"
                                                <c:if test="${address.mainAddress eq 'Y'}">disabled title="기본 배송지는 삭제할 수 없습니다."</c:if>>
                                          삭제
                                        </button>
                                      </td>
                                    </tr>
                            ...
                            <div class="row g-2">
                              ...
                              <div class="col-md-6"><input type="text" id="nickname" class="form-control" placeholder="배송지 명"></div>
                              <div class="col-md-3">
                              ...
                              <div class="col-md-3">
                                <select id="newMainAddress" class="form-select">
                                  <option value="N">기본 아님</option><option value="Y">기본 배송지</option>
                                </select>
                              </div>
                              <div class="col-12">
                                <button type="button" class="btn btn-dark w-100" onclick="addAddress()">새 배송지 등록</button>
                              </div>
                            </div>
                          </div>
                          ...
                          <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-dark" id="addressApplyBtn" data-bs-dismiss="modal">선택 적용</button>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div class="d-flex gap-2 mt-4">
                  <input type="button" id="payBtn" class="btn btn-dark" value="결제하기" onclick="showPasswordPopup()">
        ...
        <div class="col-lg-5">
          <div class="order-summary card shadow-sm" data-aos="fade-left" data-aos-delay="200">
            <div class="order-summary-header card-header d-flex justify-content-between align-items-center">
              <h3 class="mb-0">주문 요약</h3>
              <span class="item-count"><c:out value="${fn:length(orderList)}"/>개</span>
            </div>
            ...
            <div class="order-summary-content card-body">
                  ...
                  <div class="order-item d-flex gap-3">
                    ...
                    <div class="order-item-details flex-grow-1">
                      ...
                      <div class="order-item-price d-flex justify-content-between">
                        <span class="quantity text-muted">${order.orderQuantity} × ${order.price}원</span>
                        <span class="price fw-semibold">${order.orderQuantity * order.price}원</span>
                      </div>
                    </div>
                  </div>
                </c:forEach>
              ...
              <div class="order-totals vstack gap-2 border-top pt-3">
                <div class="d-flex justify-content-between"><span>총 상품 가격</span><span id="summaryTotal"><strong>${total}</strong> 원</span></div>
              ...
              <div class="secure-checkout border-top mt-3 pt-3 d-flex justify-content-between align-items-center">
                <div class="secure-checkout-header d-flex align-items-center gap-2"><i class="bi bi-shield-lock"></i><span>보안 결제</span></div>
...
<script>
/* ----- (안전) 카드번호 4칸 세팅 함수가 없을 때 에러 방지용 stub ----- */
function setupCardInputs(){ /* no-op: 분할 입력 미사용 시 무시 */ }

/* ---------- 카드 등록 ---------- */
function addCard(){
  const form = document.getElementById('newCardForm');
  if (!form) { alert('카드 입력 폼을 찾을 수 없습니다.'); return; }

  const get = sel => (form.querySelector(sel)?.value || '').trim();
  const onlyDigits = s => (s || '').replace(/\D+/g, '');
  const stripCard  = s => (s || '').replace(/[\s-]+/g, '');

  const payload = {
    userId: "${orderList[0].userId}",
    financialInstitution: get('[name="financialInstitution"]'),
    accountNumber:        stripCard(get('[name="accountNumber"]')),
    cardExpiration:       get('[name="cardExpiration"]'),
    cardCvc:              onlyDigits(get('[name="cardCvc"]')),
    cardPassword:         onlyDigits(get('[name="cardPassword"]')),
    isDefault:            (form.querySelector('#isDefault, #isDefault2')?.checked ? 'Y' : 'N')
  };

  if (!payload.financialInstitution) return alert('카드사를 입력하세요.');
  if (!payload.accountNumber || payload.accountNumber.length < 14) return alert('카드번호를 정확히 입력하세요.');
  if (!/^\d{4}-\d{2}$/.test(payload.cardExpiration)) return alert('유효기간은 YYYY-MM 형식입니다.');
  if (!/^\d{3,4}$/.test(payload.cardCvc)) return alert('CVC를 정확히 입력하세요.');
  if (!/^\d{2}$/.test(payload.cardPassword)) return alert('비밀번호 앞 2자리를 입력하세요.');

  const btn = form.querySelector('button[onclick*="addCard"]');
  if (btn) { btn.disabled = true; btn.dataset._old = btn.innerHTML; btn.innerHTML = '등록 중…'; }

  $.ajax({
    type: 'POST',
    url: '<c:url value="/personal/payment/addCard"/>',
    contentType: 'application/json; charset=UTF-8',
    dataType: 'text',
    data: JSON.stringify(payload),
    success: function(text){
      const res = (text || '').trim().toLowerCase();
      if (res === 'success'){ alert('카드가 등록되었습니다.'); location.reload(); }
      else { alert('카드 등록에 실패했습니다.'); }
    },
    error: function(xhr){
      console.error('addCard error', { status:xhr.status, response:xhr.responseText });
      alert('서버 통신 중 오류가 발생했습니다.');
    },
    complete: function(){ if (btn){ btn.disabled = false; btn.innerHTML = btn.dataset._old || '카드 등록'; } }
  });
}
...
function renderAddressRows(list){
  ...
  list.forEach(a=>{
    ...
    +   '<td>' + (isMain
            ? '✔'
            : '<button type="button" class="btn btn-outline-primary btn-sm btn-set-main" data-id="' + a.addressNo + '">기본지로</button>')
        + '</td>'
    +   '<td>'
    +     '<button type="button" class="btn btn-outline-danger btn-sm btn-del-address" data-id="' + a.addressNo + '"'
    +       (isMain ? ' disabled title="기본 배송지는 삭제할 수 없습니다."' : '') + '>삭제</button>'
    +   '</td>'
    + '</tr>';
  });
...
/* ---------- 기본 배송지 설정 ---------- */
function setMainAddress(addressNo, btn){
  if (!addressNo) return;
  btn && (btn.disabled = true);

  fetch('<c:url value="/api/address/main"/>', {
    method: 'POST',
  ...
  .then(json => {
    if (json && json.status === 'success'){
      ...
      const cb  = row.querySelector("input[name='selectedAddressChk']");
      const addr = (cb?.dataset.address || '') + ' ' + (cb?.dataset.detail || '');
      const nick = cb?.dataset.nickname || '';
      document.getElementById('selectedAddress').innerText = addr.trim();
      document.getElementById('selectedAddressNickname').innerText = nick;
      document.getElementById('selectedAddressId').value = String(addressNo);

      refreshAddressTable();
      alert('기본 배송지가 변경되었습니다.');
    }else{
      alert((json && json.message) ? json.message : '기본 배송지 변경 실패');
    }
  })
  .catch(err => {
    console.error('setMainAddress error:', err);
...
function refreshAddressTable(){
  const url = '<c:url value="/api/address/list"/>';
  const tbody = document.getElementById("addressTbody");
  if (tbody) tbody.innerHTML = '<tr><td colspan="7" class="text-muted">불러오는 중…</td></tr>';
  ...
  fetch(url, { headers:{ 'Accept': 'application/json' } })
    ...
    .then(json => {
      ...
      if (!list.length){
        if (tbody) tbody.innerHTML = '<tr><td colspan="7" class="text-muted">등록된 배송지가 없습니다.</td></tr>';
        return;
      }
      renderAddressRows(list);
    })
    .catch(err => {
      console.error('refreshAddressTable error:', err);
      if (tbody) tbody.innerHTML = '<tr><td colspan="7" class="text-danger">배송지 목록을 불러오지 못했습니다.</td></tr>';
    });
}
...
function addAddress(){
  ...
  fetch('<c:url value="/api/address/add"/>', {
    method:"POST",
    headers:{ "Content-Type":"application/json" },
...
/* ---------- 배송지 삭제 ---------- */
function deleteAddressById(addressNo, btn){
  if (!addressNo) return;
  if (!confirm('이 배송지를 삭제하시겠습니까?')) return;

  if (btn) btn.disabled = true;

  fetch('<c:url value="/api/address/delete"/>', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ addressNo: Number(addressNo) })
  })
  ...
  .then(json => {
    if (json && json.status === 'success'){
      // 현재 선택된 배송지를 지웠다면 선택 영역 초기화
      const selId = document.getElementById('selectedAddressId').value;
      if (String(selId) === String(addressNo)){
        document.getElementById('selectedAddress').innerText = '';
        document.getElementById('selectedAddressNickname').innerText = '';
...
document.addEventListener('click', function(e){
  const setBtn = e.target.closest('.btn-set-main');
  if (setBtn){
    const id = setBtn.dataset.id;
    if (id && !setBtn.disabled) setMainAddress(id, setBtn);
    return;
  }
  const delBtn = e.target.closest('.btn-del-address');
  if (delBtn){
    const id = delBtn.dataset.id;
    if (id && !delBtn.disabled) deleteAddressById(id, delBtn);
  }
});

/* ---------- 선택 적용 ---------- */
// 선택 적용: 값 반영, 선택 없으면 닫힘 막기
document.getElementById('addressApplyBtn').addEventListener('click', function (e) {
  const sel = document.querySelector("input[name='selectedAddressChk']:checked");
  if (!sel) {
    e.preventDefault(); // data-bs-dismiss 취소
    e.stopPropagation();
    alert("배송지를 선택하세요.");
    return;
  }
  document.getElementById("selectedAddress").innerText =
    (sel.dataset.address || '') + ' ' + (sel.dataset.detail || '');
  document.getElementById("selectedAddressNickname").innerText = sel.dataset.nickname || '';
  document.getElementById("selectedAddressId").value = sel.value;
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addressModal.addEventListener('show.bs.modal', refreshAddressTable);

  // 페이지 처음에도 1회 로드(SSR 데이터가 없을 때 대비)
  refreshAddressTable();
});
